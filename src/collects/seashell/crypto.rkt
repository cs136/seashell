...
(: seashell-crypt-make-key (-> Bytes))
(define (seashell-crypt-make-key)
...
(: seashell-crypt-make-token (-> Bytes))
(define (seashell-crypt-make-token)
...
(: seashell-encrypt (-> Bytes Bytes Bytes (Values Bytes Bytes Bytes)))
(define (seashell-encrypt key frame plain)
...
;; Returns:
;;  Decrypted frame.
(: seashell-decrypt (-> Bytes Bytes Bytes Bytes Bytes Bytes))
(define (seashell-decrypt key iv tag coded plain)
  (define auth (bytes-append iv plain))
...
(: seashell-crypt-key->client (-> Bytes JSExpr))
(define (seashell-crypt-key->client key)
...
;; else currently is IV, ciphertext, extra authenticated data,
;; GCM authentication tag, and plaintext.
(: seashell-crypt-key->server (-> Bytes Bytes))
(define (seashell-crypt-key->server key)
  key)
...
(: seashell-crypt-key-server-read (-> Input-Port Bytes))
(define (seashell-crypt-key-server-read port)
  (define result (read-bytes 16 port))
  (assert (bytes? result))
  result)
