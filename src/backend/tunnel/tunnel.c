...
#define HOSTS_FILE "/etc/seashell_hosts"
#define DUMP_FILE  "/tmp/seashell_hosts"
#define SEASHELL_TARGET (INSTALL_PREFIX "/bin/seashell-backend")
...
struct seashell_connection* seashell_tunnel_connect_password (const char* host,
  ...
  if (e) {
    fprintf(stderr, "%s: Error authenticating: %d\n", user, e);
...
int main (int argc, char *argv[]) {
  ...
  fprintf(stderr, "%s: Launching tunnel!\n", argv[1]);
  ...
  fprintf(stderr, "%s: Read authentication packet length.\n", argv[1]);

  if (1 != read(0, &method, 1)) {
    fprintf(stderr, "%s: I/O error on reading authentication method.\n", argv[1]);
    return IO_ERROR;
  }
  fprintf(stderr, "%s: Read authentication method.\n", argv[1]);
  ...
  fprintf(stderr, "%s: Read authentication data (%d).\n", argv[1], length);
  ...
  write(1, &success, 1);
  fprintf(stderr, "%s: Tunnel launched!\n", argv[1]);
  ...
  fprintf(stderr, "%s: Remote end hung up with %d (%s)\n", argv[1], error, exitsignal);
