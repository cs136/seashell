...
angular.module('seashell-projects', ['seashell-websocket', 'marmoset-bindings'])
  ...
  .service('projects', ['$rootScope', '$q', 'socket', 'marmoset', '$http', '$cookies',
    function($scope, $q, ws, marmoset, $http, $cookies) {
      ...
      var WL_PROJ_ZIP_URL_TEMPLATE = "cs136@linux.student.cs.uwaterloo.ca:/u2/cs136/seashell-support-files/whitelist-skeletons/%s-seashell.zip";
          ...
          if (! self._inSkeleton) {
            self._inSkeleton = $q.all([listSkelProjects(), userWhitelist(), projectWhitelist()])
              .then(function(result) {
                ...
                if(ans) ans = "public";
                if(!ans && _.find(users, function(u) { return u === USERNAME; })) {
