...
class Project extends React.Component<ProjectProps&actionsInterface, ProjectState> {
  ...
  generateMarmosetButton(project: appStateReducerProjectState) {
      ...
      if (project.currentQuestion && project.name.match(projectPattern) && project.currentQuestion.name.match(questionPattern)) {
        const marmosetDispatch = ((projectName: string, questionName: string) => {
            this.props.dispatch.question.getMarmosetResults(projectName, questionName).then(console.warn);
                ...
                navRight={question ? [
                    ...
                    <Tooltip key="project-build-file" content="Test" position={Position.BOTTOM_RIGHT}>
                        {this.props.appState.runState !== 0 || question.runFile === "" ?
                            ...
                            <button className="pt-button pt-minimal pt-icon-comparison"
                                onClick={() =>
                                    this.props.dispatch.file.flushFileBuffer().then(this.props.dispatch.compile.compileAndRun.bind(this, project.id, question.name, question.runFile, true))}>
                            </button>}
                    </Tooltip>,
                    !question.runFile ?
                    ...
                    : this.props.appState.runState === 0 ?
                        ...
                        <button className="pt-button pt-minimal pt-icon-play"
                            onClick={() => this.props.dispatch.file.flushFileBuffer().then(this.props.dispatch.compile.compileAndRun.bind(this, project.id, question.name, question.runFile, false))}>
                        </button>
                        </Tooltip>
...
>>>>>>> 98e8e709990cbf43998dc3e095737fab3064ecfd
    } else {
      return <Loading/>;
    }
  }
