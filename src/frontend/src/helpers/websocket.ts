import {SeashellWebsocket,WebsocketResult} from './websocket_client';
import {History,Settings,Test,SeashellFile,Storage,SeashellCompiler,SeashellRunner,SeashellTester,SeashellPID,Change} from './types';

class Callback {
  constructor(public type: string, public cb: (message?: any) => any, public now: boolean) { }
}

// TODO this should probably extend some general error type
...
export class WebsocketService {
  ...
  public newProject: (name: string)=>Promise<WebsocketResult>;
  ...
  public getProjects: ()=>Promise<WebsocketResult>;
  ...
  public async connect(): Promise<boolean> {
    ...
    try {
      ...
      ()=>{
        this.connected = false;
        clearInterval(this.timeoutInterval);
        this.invoke_cb_failure_om_wrap('disconnected'); });
        */
    } catch(e) {
      this.failed = true;
      this.invoke_cb_failure_om_wrap('failed');
  ...
  public isOffline(): boolean {
    return this.offlineMode === 2 || (!this.connected && this.offlineMode === 1);
  }

  public offlineEnabled(): boolean {
    return this.offlineMode === 1 || this.offlineMode === 2;
  }
  ...
  private make_offline_noop(name: string) {
    return async (...args: any[])=>{
      if(!this.isOffline()) {
        return await this.socket[name].apply(this.socket, args);
      } else {
        return true;
      }
    };
  ...
  private make_offline_enabled(name: string, offlineWriteThrough?: boolean) {
    var online_arity = this.socket[name].length;
    var offline_arity = this.storage[name].length;
    if(offline_arity != online_arity && offline_arity != online_arity+1) {
      throw new WebsocketError("Offline and online arities differ.",
        {offline: offline_arity, online:online_arity});
    }
    ...
    return async (...args: any[])=>{
      ...
      if(this.offlineEnabled()) {
        var init = await this.storage.isInitialized();
        if(!init && !this.isOffline()) {
          await this.syncAll();
        } else if(!init) {
          throw new WebsocketError("Cannot call "+name+" in offline mode before an initial sync.");
        }
        var res = await this.storage[name].apply(this.storage, args);
      }
      else {
        return await this.socket[name].apply(this.socket, args);
      }
  ...
  public async compileAndRunProject(project: string, question: string, file: SeashellFile, tests: Array<Test>) {
    if(!this.isOffline()) {
      ...
      return await this.socket.compileAndRunProject(project, question, test_names);
    }
    else {
      if(file.ext() == 'rkt') {
  ...
  public async programKill(pid: SeashellPID) {
    if(typeof pid === 'object') {
      ...
      return await this.socket.programKill(pid);
    }
  }
  ...
  public async sendEOF(pid: number|SeashellPID) {
    if(typeof pid === 'object') {
      ...
      return await this.socket.sendEOF(pid);
    }
  }
  ...
  public async startIO(project: string, pid: number|SeashellPID) {
    if(typeof pid === 'object') {
      ...
      return await this.socket.startIO(project, pid);
    }
  }
