var Format = (function () {
    ...
    function formatIncludes(code) {
        return code.replace(/#include < ([^ ]*) >/g, "#include <$1>");
    }
    ...
    return {
        formattedCode: function () {
            if (editor.somethingSelected()) {
                ...
                editor.replaceSelection(formatIncludes(js_beautify(editor.getRange(from, to))));
            } else {
                editor.setValue(formatIncludes(js_beautify(editor.getValue())));
            }
            return formatIncludes(js_beautify(code));
        },
        ...
        commentLines: function() {
            ...
            var endLine;
            if (editor.somethingSelected()) {
                startLine = editor.getCursor("start").line;
                endLine = editor.getCursor("end").line;
            } else {
                var cursorPos = editor.getCursor().line;
                startLine = cursorPos;
                endLine = cursorPos;
            }
            var shouldComment = editor.getLine(startLine).trim().substring(0, 2) != "//";
...
(function() {
    ...
    function Beautifier(js_source_text, options) {
        ...
        line_starters = 'continue,try,throw,return,var,if,switch,case,default,for,while,break,function,#include'.split(',');
